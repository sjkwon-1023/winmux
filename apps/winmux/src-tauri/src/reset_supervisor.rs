//! 자동 UI 리셋 supervisor — `winmux_core::reset::ResetPolicy` 의 글루 (계획 16단계
...
fn config_from_env() -> ResetConfig {
    ...
    let idle_secs = env_u64("WINMUX_RESET_IDLE_SECS", 1800);
    let hidden_secs = env_u64("WINMUX_RESET_HIDDEN_SECS", 600);
    ...
    let cooldown_secs = env_u64("WINMUX_RESET_COOLDOWN_SECS", 300);
