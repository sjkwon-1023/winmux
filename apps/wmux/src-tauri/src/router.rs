...
use wmux_core::notify::OscBatch;
use wmux_core::osc::OscEvent;
use wmux_core::session::SessionId;
...
/// `WMUX_OSC_FLUSH_MS` → flush 창 (`WMUX_RESET_*` knob 과 같은 규율: 미설정은
...
fn flush_window_from_env() -> Duration {
    let ms = match std::env::var("WMUX_OSC_FLUSH_MS") {
        Err(std::env::VarError::NotPresent) => DEFAULT_FLUSH_MS,
        Err(err) => {
            eprintln!(
                "[wmux] osc: WMUX_OSC_FLUSH_MS unreadable ({err}); \
        ...
        Ok(raw) => match raw.trim().parse::<u64>() {
            ...
            Err(_) => {
                eprintln!(
                    "[wmux] osc: WMUX_OSC_FLUSH_MS={raw:?} is not a non-negative integer; \
                     using default {DEFAULT_FLUSH_MS}"
                );
                DEFAULT_FLUSH_MS
            }
...
impl OscRouter {
    ...
    pub fn spawn(app: AppHandle) -> Self {
        ...
        eprintln!("[wmux] osc: flush window {} ms", window.as_millis());
    ...
    fn drop(&mut self) {
        ...
        if let Some(worker) = self.worker.take() {
            if worker.join().is_err() {
                eprintln!("[wmux] osc: router worker thread panicked");
            }
        }
    }
}
...
fn apply_batch(inner: &RouterInner, batch: OscBatch) {
    ...
    let Some(managed) = inner.app.try_state::<AppState>() else {
        ...
        // 기록만 남긴다.
        eprintln!("[wmux] osc: managed state unavailable; batch dropped");
        return;
    };
