...
pub fn load(path: &Path) -> LoadOutcome {
    ...
    match value.get("version").and_then(|v| v.as_u64()) {
        ...
        Some(found) if found != u64::from(PERSIST_VERSION) => {
            ...
            eprintln!(
                "[wmux] persist: unsupported state version {found} (expected {PERSIST_VERSION}) — starting fresh"
            );
...
fn fresh_corrupt(path: &Path, error: String) -> LoadOutcome {
    let backup = backup_corrupt(path);
    eprintln!(
        "[wmux] persist: state file corrupt ({}): {error} — starting fresh",
        path.display()
    );
...
impl Saver {
    ...
    pub fn schedule(&self, state: AppState) {
        ...
        if tx.send(SaverMsg::Schedule(Box::new(state))).is_err() {
            ...
            eprintln!("[wmux] persist: saver worker is gone; schedule dropped");
        }
    }
    ...
    pub fn flush(&self) {
        ...
        if tx.send(SaverMsg::Flush(ack_tx)).is_err() {
            eprintln!("[wmux] persist: saver worker is gone; flush dropped");
            return;
        }
        if ack_rx.recv().is_err() {
            eprintln!("[wmux] persist: saver worker died before flush ack");
        }
    }
}
    ...
    fn drop(&mut self) {
        ...
        if let Some(worker) = self.worker.take() {
            if worker.join().is_err() {
                eprintln!("[wmux] persist: saver worker thread panicked");
            }
        }
    }
}
...
fn write_pending(path: &Path, pending: &mut Option<Box<AppState>>) {
    if let Some(state) = pending.take() {
        if let Err(err) = save_atomic(path, &state) {
            eprintln!(
                "[wmux] persist: state save failed ({}): {err}",
                path.display()
            );
        }
    }
