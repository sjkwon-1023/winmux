...
//! # OSC 의미 규약
//!
//! - OSC 777 `notify;title;body` 의 title 이 `wmux:running` | `wmux:needsInput` |
//!   `wmux:idle` 이면 **상태 알림**: 해당 [`AgentStatus`] + body(비어있지 않으면)를
...
pub struct OscDelta {
    ...
    /// `wmux:` 토큰이 붙은 777 의 상태 — last-wins.
...
/// OSC 777 title 의 `wmux:` 상태 토큰을 파스한다. 규약 외 문자열은 `None`
/// (= 상태 중립 알림).
fn parse_status_token(title: &str) -> Option<AgentStatus> {
    match title {
        "wmux:running" => Some(AgentStatus::Running),
        "wmux:needsInput" => Some(AgentStatus::NeedsInput),
        "wmux:idle" => Some(AgentStatus::Idle),
        _ => None,
    }
}

/// OSC 7 의 `file://host/path` 에서 경로만 뽑아 percent-decode 한다.
/// host 부분은 무시한다 — wmux 는 자기 PTY 가 보고한 경로만 쓰므로 호스트명이 무엇이든
...
mod tests {
    ...
    fn message_is_last_non_empty() {
        ...
        let delta = merged(&[
            notify("wmux:needsInput", "approve?"),
            notify("wmux:running", ""),
        ]);
    ...
    fn running_alone_sets_no_unread() {
        let delta = merged(&[notify("wmux:running", "working")]);
        assert_eq!(delta.status, Some(AgentStatus::Running));
        assert!(!delta.unread);
    }
    ...
    fn needs_input_and_idle_set_unread() {
        assert!(merged(&[notify("wmux:needsInput", "")]).unread);
        assert!(merged(&[notify("wmux:idle", "done")]).unread);
    }
    ...
    #[test]
    fn unknown_wmux_token_is_status_neutral() {
        // 규약 밖 제목(다른 도구의 777)은 상태를 주장하지 못하고 알림만 남긴다.
        let delta = merged(&[notify("Build", "finished"), notify("wmux:bogus", "")]);
        assert_eq!(delta.status, None);
        assert!(delta.unread);
    ...
    fn message_truncated_at_multibyte_boundary() {
        ...
        let delta = merged(&[notify("wmux:idle", &body)]);
    ...
    fn sessions_are_independent() {
        let mut batch = OscBatch::default();
        batch.merge(1, &notify("wmux:needsInput", "a"));
    ...
    fn flood_of_events_keeps_one_slot_per_session() {
        ...
        for i in 0..1000 {
            ...
            batch.merge(7, &notify("wmux:running", "x"));
        }
    ...
    fn take_empties_the_batch() {
        let mut batch = OscBatch::default();
        batch.merge(1, &notify("wmux:idle", "done"));
        assert!(!batch.is_empty());
